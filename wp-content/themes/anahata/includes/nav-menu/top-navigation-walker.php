 ...
 */
if(!class_exists('AnahataMikadoTopNavigationWalker')) {
	class AnahataMikadoTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.anahata_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if($item->hide == '') {
				...
				if($depth == 0) {
					$angle_icon_first_level = 'arrow_carrot-down';
				}
				...
				if(!empty($icon) && anahata_mikado_get_meta_field_intersect('header_type', anahata_mikado_get_page_id()) !== 'header-vertical') {
				...
				if(!empty($item->description) && anahata_mikado_get_meta_field_intersect('header_type', anahata_mikado_get_page_id()) !== 'header-vertical') {
				...
				if($args->has_children && $angle_icon_first_level != "") {
					$item_output .= '<i class="mkd_menu_arrow '.$angle_icon_first_level.'"></i>';
				}
				...
				$item_output .= '</span><span class="menu_line"></span></a>';
			}
