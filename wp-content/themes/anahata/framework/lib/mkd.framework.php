...
class AnahataMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions   = AnahataMikadoOptions::get_instance();
		$this->mkdMetaBoxes = AnahataMikadoMetaBoxes::get_instance();
	}
	...
	public function setSkin(AnahataMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AnahataMikadoSkinManager
 *
...
/**
 * Class AnahataMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AnahataMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	/**
	 * Loads template part with params. Uses anahata_mikado_find_template_path function which is child theme friendly
	 *
	...
	public function loadTemplatePart($template, $params = array()) {
		...
		if($template !== '') {
			include(anahata_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo anahata_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class AnahataMikadoOptions {
	...
	public function getOptionValue($key) {
		...
		if(array_key_exists($key, $anahata_options)) {
			return $anahata_options[$key];
...
/*
   Class: AnahataMikadoAdminPage
   A class that initializes Mikado Admin Page
*/

class AnahataMikadoAdminPage implements iAnahataMikadoLayoutNode {
	...
	function __construct($slug = "", $title_label = "", $icon = "") {
		$this->layout  = array();
		$this->factory = new AnahataMikadoFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild(iAnahataMikadoRender $child) {
		$child->render($this->factory);
	}
}

/*
   Class: AnahataMikadoMetaBoxes
...
/*
   Class: AnahataMikadoMetaBox
   A class that initializes Mikado Meta Box
*/

class AnahataMikadoMetaBox implements iAnahataMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $anahata_Framework->getSkin()->getSkinLabel().' '.$label;
	}
}

global $anahata_Framework;

$anahata_Framework = AnahataMikadoFramework::get_instance();
$mkdSkinManager                 = new AnahataMikadoSkinManager();
$anahata_Framework->setSkin($mkdSkinManager->getSkin());
